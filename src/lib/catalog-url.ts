...
/**
 * `sort=popular` is the pre-★ spelling of `standard`, from the same era as
 * `pop=` above: read here so an old link keeps its meaning, never written
 * again. Any other unrecognised value — including a bare `?view=all` link's
 * absence of `sort` — falls back to "recommended", the current default.
 */
function readLegacySort(raw: string | null): SortKey {
  return raw === "popular" ? "standard" : "recommended";
}
...
export function readFilters(params: URLSearchParams | ReadonlyURLSearchParams): {
  ...
  return {
    ...
    sort:
      sort === "standard" || sort === "title" || sort === "composer"
        ? sort
        : readLegacySort(sort),
  };
}
...
export function writeFilters(filters: CatalogFilters, sort: SortKey): string {
  ...
  // "recommended" is the default, so it is the one value never written —
  // same convention as every other default-valued field above.
 ...
 * reaches `router.replace()` unchecked. Unknown keys — including a legacy
 * `view=all`, now that the catalogue has only one view — are dropped, and
 * enum values are validated by `readFilters`; running the result back
 * through `writeFilters` means anything malformed — including something
 * that is not a query string at all, like `//evil.com` — collapses to `""`.
 */
export function sanitizeQueryString(raw: string | null): string {
  if (!raw) return "";
  const { filters, sort } = readFilters(new URLSearchParams(raw.replace(/^\?/, "")));
